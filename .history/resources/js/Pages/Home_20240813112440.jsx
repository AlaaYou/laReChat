...
function Home({ selectedConversation = null, messages = null }) {
        ...
        axios.get(route("message.loadOlder", firstMessage.id))
            .then(({ data }) => {
                ...
                setLocalMessages(prevMessages => {
                    // Adjust scroll position if not an initial load
                    const scrollTop = messagesCtrRef.current.scrollTop;
                    const newMessages = [...data.data.reverse(), ...prevMessages];
                    setTimeout(() => {
                        messagesCtrRef.current.scrollTop = messagesCtrRef.current.scrollHeight - (scrollTop + messagesCtrRef.current.clientHeight);
                    }, 0);
                    return newMessages;
                });
            });
    ...
    useEffect(() => {
        ...
        setIsInitialLoad(true); // Set to true when messages are initially loaded
    }, [messages]);

    useEffect(() => {
        if (messagesCtrRef.current) {
            if (isInitialLoad) {
                // Reset scroll to bottom when first loading messages or opening a new conversation
                messagesCtrRef.current.scrollTop = messagesCtrRef.current.scrollHeight - messagesCtrRef.current.clientHeight;
