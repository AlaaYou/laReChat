...
const ConversationHeader = ({ selectedConversation, onConversationDeleted }) => {
    ...
    const onDeleteGroup = () => {
        ...
        axios.delete(route("group.destroy", selectedConversation.id))
            .then((res) => {
                ...
                emit("ConversationHeader.groupDeleted", selectedConversation.id);
            })
            .catch((err) => {
    ...
    useEffect(() => {
        const handleGroupDeleted = (groupId) => {
            if (selectedConversation.id === groupId) {
                // Trigger parent component to update its state
                onConversationDeleted();
            }
        };
        
        on("ConversationHeader.groupDeleted", handleGroupDeleted);

        return () => {
            off("ConversationHeader.groupDeleted", handleGroupDeleted);
        };
    }, [selectedConversation, onConversationDeleted]);
